...
class TestRequirePropertyScopeQueryParam:
    ...
    async def test_staff_with_query_scope_row_passes(self) -> None:
        ...
        with pytest.MonkeyPatch().context() as mp:
            mp.setattr(
                "app.modules.auth.repository.UserRepository.get_property_scopes",
                fake.get_property_scopes,
            )
            await enforce(current_user, request, AsyncMock())  # must not raise
    ...
    async def test_owner_bypasses_without_scope_row(self) -> None:
        """Owner bypasses even if no scope row exists for property."""
        dep = require_property_scope(query_param="property_id")
        enforce = dep.dependency  # type: ignore[attr-defined]
...
# ── #5 regression guards: body/path sourced scopes must still work ─────


@pytest.mark.unit
class TestRequirePropertyScopeBodyPath:
    """Body/path sourced ``require_property_scope()`` still enforced."""

    async def test_body_sourced_enforces_on_post_create(self) -> None:
        """POST / create tenant enforces body-sourced property_id."""
...
@pytest.mark.unit
class TestSearchByLiteral:
    """``search_by`` restricted to Literal["name","phone","email"] (#7)."""
...
class TestSearchCacheControl:
    ...
    async def test_search_sets_no_store_cache_control(self) -> None:
        ...
        with pytest.MonkeyPatch().context() as mp:
            ...
            mp.setattr(
                tenant_router_module,
                "require_property_scope",
                lambda *_, **__: lambda: None,
            )
